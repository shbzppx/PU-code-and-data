...
def split_minerals_leave_one_camp(
    mineral_df,
    *,
    n_clusters: int = 4,
    ...
    train_root = _Path(__file__).resolve().parents[1] / "模型训练"
    if str(train_root) not in sys.path:
        sys.path.append(str(train_root))
    from spatial_validation_splits import split_leave_one_camp

    return split_leave_one_camp(
        mineral_df,
        n_clusters=n_clusters,
        held_out_camp_index=held_out_camp_index,
        random_state=random_state,
    )


def split_minerals_leave_one_fault(
    mineral_df,
    *,
    ...
    if str(train_root) not in sys.path:
        sys.path.append(str(train_root))
    from spatial_validation_splits import split_leave_one_fault

    return split_leave_one_fault(
        mineral_df,
        held_out_fault_id=held_out_fault_id,
        assignment=assignment,
    )


def split_minerals_variogram_block(
    mineral_df,
    *,
    ...
    if str(train_root) not in sys.path:
        sys.path.append(str(train_root))
    from spatial_validation_splits import split_variogram_block_cv

    return split_variogram_block_cv(
        mineral_df,
        variogram_range_m=variogram_range_m,
        held_out_block_index=held_out_block_index,
        random_state=random_state,
    )
