...
TRAIN_DIR = os.path.join(CODE_ROOT, "模型训练")
for path in (CODE_ROOT, COMMON_DIR, TRAIN_DIR):
...
from nested_fault_calibration import (
    find_fault_channel_indices,
    should_drop_distance_after_nested,
)

from model import (LinearClassifier, ThreeLayerPerceptron, MultiLayerPerceptron,
                   CNN, PUCNN, NNPUCNN, RNAdaptivePUCNN, RNConfidenceAdaptivePUCNN, RNConfidenceStaticPUCNN,
                   RNFullCoverageAdaptivePUCNN, RNFullCoverageStaticPUCNN,
                   RNGapAdaptivePUCNN, RNGapStaticPUCNN,
                   RNLogitRankAdaptivePUCNN, RNLogitRankStaticPUCNN,
                   RNSpatialConservativeAdaptivePUCNN, RNSpatialConservativeStaticPUCNN,
                   CNNTransformer, CNNTokenTransformer, RandomForestBinaryClassifier,
                   OneClassSVMClassifier, PURandomForestClassifier, TwoStepPULearning)
import h5py
from datetime import datetime
import pickle # <-- 确保在文件顶部导入 pickle
from spatial_score_fusion import apply_model_spatial_score_mode
try:
    import pandas as pd
    import math
...
def _extract_metadata(handle):
    ...
    for key in (
        ...
        "coordinate_unit_to_meters", "coordinate_step", "coordinate_step_units",
        "grid_spacing_coordinate_units", "coordinate_grid_spacing", "cell_size_m",
    ):
...
def load_model(model_name, model_path, prior, input_dim, device, data_shape=None):
    ...
    models = {
        ...
        "pucnn": PUCNN,
        "nnpucnn": NNPUCNN,
        "rnapucnn": RNAdaptivePUCNN,
        "rncapucnn": RNConfidenceAdaptivePUCNN,
        "rncpucnn": RNConfidenceStaticPUCNN,
        "rnfcapucnn": RNFullCoverageAdaptivePUCNN,
        "rnfcspucnn": RNFullCoverageStaticPUCNN,
        "rngapucnn": RNGapAdaptivePUCNN,
        "rngspucnn": RNGapStaticPUCNN,
        "pucnnt": CNNTransformer,
        "pucnntransformer": CNNTokenTransformer,
        'rnlrapucnn': RNLogitRankAdaptivePUCNN,
        'rnlrspucnn': RNLogitRankStaticPUCNN,
        'rnscapucnn': RNSpatialConservativeAdaptivePUCNN,
        'rnscspucnn': RNSpatialConservativeStaticPUCNN,
    ...
    image_model_names.update({'rnlrapucnn', 'rnlrspucnn', 'rnscapucnn', 'rnscspucnn'})
    ...
    if model_name in image_model_names:
        ...
        # 其他CNN模型的原有逻辑
            ...
            if model_name in cnn_like_models:
                # CNN: fc1 入维 = 128 * final_size^2；3 次池化 => 原始边长约 final_size*8
                # 注意: 这只是启发式提示，真实形状以 checkpoint/数据为准；通道数也不固定为 11。
            ...
            # 计算原始输入维度（仅用于日志对照；通道未知时不硬编码为 11）
            inferred_channels = resolved_shape[0] if resolved_shape[0] is not None else None
            if inferred_channels is not None:
                orig_input_dim = inferred_channels * orig_img_size * orig_img_size
            else:
                orig_input_dim = None
            
            print(f"检测到模型原始输入尺寸(启发式): {orig_img_size}x{orig_img_size}")
            if resolved_shape[1] is not None and resolved_shape[2] is not None:
                print(f"当前数据窗口尺寸: {resolved_shape[1]}x{resolved_shape[2]}")
            else:
                print(f"当前请求的输入维度: {input_dim}")
            
            # 如果输入维度不匹配，优先使用请求的输入维度（实际数据的维度）
            if orig_input_dim is not None and orig_input_dim != input_dim:
                print("提示: 启发式推算尺寸与实际数据维度不同（常见于 fc 入维推算），将以实际数据形状加载权重。")
                print(f"将使用实际数据维度: {input_dim}（启发式对照: {orig_input_dim}）")
    ...
    # 加载状态（预测阶段禁止在权重失败时静默使用随机初始化模型）
    try:
        model.load_state_dict(state_dict)
        print("成功加载模型权重")
    except RuntimeError as e:
        raise RuntimeError(
            f"加载模型权重失败，已中止预测（避免使用未初始化权重导致满屏高概率假图）: {e}"
        ) from e
...
def predict_single(model, data, device):
    ...
    with torch.no_grad():
        ...
        confidence = (
            score_transform(output) if callable(score_transform) else torch.sigmoid(output)
        ).item()
...
def _infer_expected_input_channels(model_path, patch_size=None):
    """Best-effort channel count from checkpoint / sklearn pickle."""
    path = str(model_path or "")
    lower = path.lower()
    patch = int(patch_size or 0) or None
    try:
        if lower.endswith(".pkl"):
            with open(path, "rb") as handle:
                obj = pickle.load(handle)
            feature_mean = getattr(obj, "feature_mean", None)
            if feature_mean is not None:
                flat = int(np.asarray(feature_mean).reshape(-1).shape[0])
                if patch and flat % (patch * patch) == 0:
                    return flat // (patch * patch)
            input_dim = getattr(obj, "input_dim", None)
            if input_dim is not None and patch and int(input_dim) % (patch * patch) == 0:
                return int(input_dim) // (patch * patch)
            return None
        if lower.endswith((".pth", ".pt")):
            ckpt = _torch_load_compat(path, map_location="cpu")
            if isinstance(ckpt, dict):
                if ckpt.get("input_channels") is not None:
                    return int(ckpt["input_channels"])
...
def _sklearn_expected_flat_dim(model):
    feature_mean = getattr(model, "feature_mean", None)
    if feature_mean is not None:
        return int(np.asarray(feature_mean).reshape(-1).shape[0])
    input_dim = getattr(model, "input_dim", None)
    if input_dim is not None:
        return int(input_dim)
    estimator = getattr(model, "reliable_negatives_estimator", None) or getattr(
        model, "model", None
    )
...
def _align_batch_to_sklearn_model(model, data):
    """Flatten image batches and drop nested fault-distance channel when needed."""
    if torch.is_tensor(data):
        arr = data.detach().cpu().numpy()
    else:
        arr = np.asarray(data)
    expected = _sklearn_expected_flat_dim(model)
    if arr.ndim == 4:
        n_samples, channels, height, width = arr.shape
        flat_dim = int(channels * height * width)
        if expected is not None and flat_dim != expected:
            channel_delta = flat_dim - int(expected)
            if channel_delta == int(height * width) and channels >= 2:
                # One extra full channel vs the fitted model. Prefer dropping the
                # Euclidean-distance channel that sits immediately before decay.
                drop_idx = channels - 2
                keep = [i for i in range(channels) if i != drop_idx]
                arr = arr[:, keep, ...]
                print(
                    f"警告: 预测张量 {channels}x{height}x{width} 与模型特征维 {expected} 不一致，"
                    f"已丢弃通道 index={drop_idx} 以对齐 fault_decay_only 训练口径。"
                )
            else:
                raise ValueError(
                    f"sklearn 模型期望特征维 {expected}，但收到 "
                    f"{channels}x{height}x{width}={flat_dim}；请检查断裂消融通道是否与训练一致。"
                )
        arr = arr.reshape(arr.shape[0], -1)
    elif arr.ndim == 2 and expected is not None and arr.shape[1] != expected:
        raise ValueError(
            f"sklearn 模型期望特征维 {expected}，但收到 {arr.shape[1]}。"
        )
    return arr
...
def predict_batch(model, data, device):
    ...
    if hasattr(model, "predict") and hasattr(model, "predict_proba") and getattr(model, "is_fitted", False):
        aligned = _align_batch_to_sklearn_model(model, data)
        probabilities = np.asarray(model.predict_proba(aligned), dtype=np.float32)
        positive_confidence = probabilities[:, 1].reshape(-1)
        predictions = np.asarray(model.predict(aligned)).reshape(-1)
    ...
    with torch.no_grad():
        ...
        score_transform = getattr(model, 'positive_scores_from_logits', None)
        confidences = (
            score_transform(outputs) if callable(score_transform) else torch.sigmoid(outputs)
        ).cpu().numpy()
        # 将预测结果转换为字符串标签
...
def main():
    ...
    try:
        ...
        # 计算召回率
        position_array = torch.stack(all_positions).cpu().numpy()
        fused_confidences, applied_score_mode = apply_model_spatial_score_mode(
            model,
            position_array,
            all_confidences,
        ...
        if applied_score_mode == 'window_average':
            all_confidences = fused_confidences.tolist()
            all_predictions = [
                'Positive' if score >= 0.5 else 'Negative'
                for score in all_confidences
            ]
            print('已应用模型预注册的窗口平均空间融合。')
...
def _drop_fault_distance_channel_after_norm(windows, means, stds, metadata):
    """Match nested fault_decay_only training: normalize with distance, then drop it."""
    names = list(
        (metadata or {}).get("selected_channel_names")
        or (metadata or {}).get("available_channel_names")
        or []
    )
    dist_idx = find_fault_channel_indices(names).get("distance")
    if dist_idx is None:
        return windows, means, stds, metadata, None
    dist_idx = int(dist_idx)
    n_channels = int(windows.shape[1]) if hasattr(windows, "shape") and len(windows.shape) >= 2 else 0
    if dist_idx < 0 or dist_idx >= n_channels:
        return windows, means, stds, metadata, None

    keep = [i for i in range(n_channels) if i != dist_idx]
    windows_out = windows[:, keep, ...]
    means_out = [means[i] for i in keep] if means and len(means) == n_channels else list(means or [])
    stds_out = [stds[i] for i in keep] if stds and len(stds) == n_channels else list(stds or [])
    meta_out = dict(metadata or {})
    if names and len(names) == n_channels:
        meta_out["selected_channel_names"] = [n for i, n in enumerate(names) if i != dist_idx]
        meta_out["selected_channel_indices"] = [
            int(idx)
            for i, idx in enumerate(meta_out.get("selected_channel_indices") or list(range(n_channels)))
            if i != dist_idx
        ]
        meta_out["fault_channel_indices"] = find_fault_channel_indices(
            meta_out["selected_channel_names"]
        )
    meta_out["fault_distance_dropped_after_nested"] = True
    return windows_out, means_out, stds_out, meta_out, dist_idx
...
class PredictionDataset(Dataset):
    def __init__(
        ...
        if len(self.means) != int(self.windows.shape[1]) or len(self.stds) != int(self.windows.shape[1]):
            raise ValueError(
                f"标准化参数通道数 ({len(self.means)}) 与特征通道数 ({self.windows.shape[1]}) 不一致。"
            )

        for i in range(self.windows.shape[1]):
            self.windows[:, i] = (self.windows[:, i] - self.means[i]) / (self.stds[i] + 1e-8)

        if drop_fault_distance is None:
            drop_fault_distance = should_drop_distance_after_nested(fault_ablation)
            if (
                not drop_fault_distance
                and expected_channels is not None
                and int(self.windows.shape[1]) == int(expected_channels) + 1
            ):
                # Model was fit after nested distance drop, but params still list the
                # pre-drop channel set (common for fault_decay_only pickles).
                drop_fault_distance = True
        if drop_fault_distance:
            (
                self.windows,
                self.means,
                self.stds,
                self.metadata,
                dropped_idx,
            ) = _drop_fault_distance_channel_after_norm(
                self.windows, self.means, self.stds, self.metadata
            )
            if dropped_idx is not None:
                print(
                    f"断裂消融 {fault_ablation or 'decay_only'}: "
                    f"标准化后丢弃欧氏距离通道 (index={dropped_idx})，"
                    f"预测输入通道数={self.windows.shape[1]}"
                )
            elif expected_channels is not None and int(self.windows.shape[1]) != int(expected_channels):
                raise ValueError(
                    f"无法对齐模型通道数: 数据 {self.windows.shape[1]}，模型期望 {expected_channels}。"
                )
