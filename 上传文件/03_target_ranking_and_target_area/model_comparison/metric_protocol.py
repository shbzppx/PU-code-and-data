...
METRIC_PROTOCOL = "independent_test_v2"
...
DEFAULT_METRIC_UNIT = "deposit_unit"


def wilson_binomial_ci(successes: int, total: int, *, alpha: float = 0.05) -> Tuple[float, float, float]:
    """Wilson score interval for a binomial proportion."""
    n = int(max(total, 0))
    k = int(max(min(successes, n), 0))
    if n <= 0:
        return 0.0, float("nan"), float("nan")
    from math import sqrt

    z = 1.959963984540054  # approx for alpha=0.05
    ...
    half = (z / denom) * sqrt((phat * (1.0 - phat) + z * z / (4.0 * n)) / n)
    return float(phat), float(max(center - half, 0.0)), float(min(center + half, 1.0))


def paired_permutation_pvalue(
    primary_scores: Iterable[float],
    ...
    *,
    ...
    """Paired permutation test on repeated-run metric differences."""
    a = np.asarray(list(primary_scores), dtype=np.float64).reshape(-1)
    b = np.asarray(list(baseline_scores), dtype=np.float64).reshape(-1)
    n = int(min(len(a), len(b)))
    if n <= 0:
        return {"pvalue": float("nan"), "observed_mean_diff": float("nan"), "n": 0}
    diff = a[:n] - b[:n]
    ...
    pvalue = (count + 1.0) / (float(n_permutations) + 1.0)
    return {
        "pvalue": float(pvalue),
...
def metric_protocol_fields(
    *,
    ...
    metric_unit: str = DEFAULT_METRIC_UNIT,
) -> Dict[str, object]:
    return {
        "metric_protocol": METRIC_PROTOCOL,
        "metric_unit": str(metric_unit or DEFAULT_METRIC_UNIT),
...
def evaluate_independent_test_metrics(
    ...
    *,
    ...
    for threshold in threshold_candidates(step=threshold_step, fixed_threshold=fixed_threshold):
        ...
        sr_point, sr_ci_low, sr_ci_high = wilson_binomial_ci(hit_count, total_deposits)
        row = {
            "threshold": float(threshold),
            "test_sr": sr,
            "test_sr_ci_low": float(sr_ci_low),
            "test_sr_ci_high": float(sr_ci_high),
            "test_paf": paf,
            "test_ei": ei,
            "sr": sr,
            "pa": paf,
