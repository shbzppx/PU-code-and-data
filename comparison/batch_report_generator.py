...
class BatchReportGenerator(ReportGenerator):
    ...
    def _overview_rows(self):
        ...
        rows = [
            ...
            ["最佳划分综合分", f"{float(best_result.get('composite_score') or 0.0):.4f}" if best_result else ""],
        ...
        for item in self.batch_summary.get("run_rows") or []:
            rows.append(
                [
                    ...
                    f"{float(item.get('best_composite_score') or 0.0):.4f}" if item.get("best_composite_score") is not None else "",
        ...
        for item in self.batch_summary.get("model_rows") or []:
            rows.append(
                [
                    ...
                    item.get("run_count", ""),
                    item.get("top1_count", ""),
                    f"{float(item.get('mean_composite_score') or 0.0):.4f}" if item.get("mean_composite_score") is not None else "",
                    f"{float(item.get('std_composite_score') or 0.0):.4f}" if item.get("std_composite_score") is not None else "",
    ...
    def _run_frame(self):
        return pd.DataFrame(
            self._run_rows(),
            columns=["轮次", "seed", "最佳", "模型", "综合分", "验证Acc", "测试检出", "训练矿点", "测试矿点", "输出目录"],
        )

    def _model_frame(self):
        return pd.DataFrame(
            self._model_rows(),
            columns=["模型", "出现次数", "Top1次数", "平均综合分", "标准差", "平均验证Acc", "平均测试检出", "稳定分"],
        )
