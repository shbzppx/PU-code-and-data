...
from .analysis_utils import primary_test_ei_score, primary_test_ei_sort_key
...
class WorkflowOrchestrator(QObject):
        ...
        params.update(
            {
                ...
                "spatial_cluster_active": bool(bundle.dataset_summary.get("spatial_cluster_active", False)),
                "spatial_cluster_n_clusters": int(bundle.dataset_summary.get("spatial_cluster_n_clusters", 0) or 0),
                "spatial_cluster_train_ratio": float(bundle.dataset_summary.get("spatial_cluster_train_ratio", 0.0) or 0.0),
                "spatial_cluster_cv_folds": int(bundle.dataset_summary.get("spatial_cluster_cv_folds", 0) or 0),
                "spatial_cluster_train_mineral_count": int(bundle.dataset_summary.get("spatial_cluster_train_mineral_count", 0) or 0),
                "spatial_cluster_test_mineral_count": int(bundle.dataset_summary.get("spatial_cluster_test_mineral_count", 0) or 0),
                "spatial_cluster_random_state": int(bundle.dataset_summary.get("spatial_cluster_random_state", 0) or 0),
                "spatial_cv_enabled": bool(bundle.dataset_summary.get("spatial_cv_enabled", False)),
                "spatial_cv_fold_count": int(bundle.dataset_summary.get("spatial_cv_fold_count", 0) or 0),
                "spatial_cv_axis": bundle.dataset_summary.get("spatial_cv_axis"),
                "spatial_cv_buffer_distance": float(bundle.dataset_summary.get("spatial_cv_buffer_distance", 0.0) or 0.0),
                "spatial_cv_selected_fold": int(bundle.dataset_summary.get("spatial_cv_selected_fold", 0) or 0),
                "buffer_policy": dict(bundle.dataset_summary.get("buffer_policy") or {}),
                "buffer_exclusion_enabled": bool(bundle.dataset_summary.get("buffer_exclusion_enabled", False)),
                "buffer_exclusion_applied": bool(bundle.dataset_summary.get("buffer_exclusion_applied", False)),
                "buffer_exclusion_removed_count": int(bundle.dataset_summary.get("buffer_exclusion_removed_count", 0) or 0),
                "buffer_exclusion_distance": float(bundle.dataset_summary.get("buffer_exclusion_distance", 0.0) or 0.0),
                "test_window_overlap_exclusion_enabled": bool(bundle.dataset_summary.get("test_window_overlap_exclusion_enabled", False)),
                "test_window_overlap_exclusion_applied": bool(bundle.dataset_summary.get("test_window_overlap_exclusion_applied", False)),
                "test_window_overlap_exclusion_removed_count": int(bundle.dataset_summary.get("test_window_overlap_exclusion_removed_count", 0) or 0),
                "test_window_overlap_exclusion_distance": float(bundle.dataset_summary.get("test_window_overlap_exclusion_distance", 0.0) or 0.0),
                "evaluation_protocol": dict(bundle.dataset_summary.get("evaluation_protocol") or {}),
            }
        )
        return params
        ...
        *,
        ...
        try:
            ...
            for scheme_index, scheme in enumerate(schemes, start=1):
                ...
                scheme_score = float(np.mean([primary_test_ei_score(item) for item in results])) if results else 0.0
            ...
            best_by_composite = max(
                packaged_results,
                key=lambda item: float(item.get("composite_score", 0.0) or 0.0),
            ) if packaged_results else None
            best_by_ei = max(packaged_results, key=primary_test_ei_score) if packaged_results else None
    ...
    def _package_top_results(
        ...
        *,
        ...
        best_results = list(sorted(ranked_results, key=self._result_sort_key)[:5])

        # Materialize the top-5 configurations by the primary test-EI score.
        for rank, result in enumerate(best_results, start=1):
            ...
            result = dict(result)
            result["auto_optimization_rank"] = int(rank)
            self.log_message.emit(
                f"Packaging top-{rank} result for model {result.get('model_name')} using evaluated checkpoint: {scheme.cache_key}"
            )
            evaluator = MineralEvaluator(
                ...
                prediction_patch_stride=int(bundle.dataset_summary.get("patch_stride", bundle.dataset_summary.get("patch_size", bundle.dataset_meta.get("image_size", 1)))),
            )
    ...
    def _build_mineral_split_summary(
        ...
        *,
        ...
        if effective_split_mode in {"single", "spatial_region", "spatial_cluster"}:
            preview_limit = None
        summary = {
    ...
    def _generate_schemes(
        ...
        base_sampling_percentage = float(dataset_request["build_config"].get("sampling_percentage", 1.0) or 1.0)
        sampling_percentages = self._normalize_float_candidates(
            candidate_config.get("sampling_percentage_candidates") or [base_sampling_percentage]
        )
        ...
        spatial_cluster_hash = self._hash_spatial_cluster_split(dataset_request["build_config"].get("spatial_cluster_split"))
        buffer_policy_hash = self._hash_buffer_policy(dataset_request["build_config"].get("buffer_policy"))
        ...
        for patch_size, patch_stride, buffer_radius, sampling_percentage, balance_ratio in product(
            ...
            cache_key = (
                ...
                f"|nb={int(n_blocks)}|region={region_hash}|cluster={spatial_cluster_hash}|buffer={buffer_policy_hash}|{mineral_hash}"
            )
            schemes.append(
                DatasetScheme(
    ...
    def _hash_spatial_cluster_split(self, cluster_config: Optional[Dict[str, object]]) -> str:
        if not cluster_config:
            return "none"
        parts = [
            int(cluster_config.get("n_clusters", 0) or 0),
            float(cluster_config.get("train_ratio", 0.7) or 0.7),
            int(cluster_config.get("cv_folds", 5) or 5),
            int(cluster_config.get("random_state", 42) or 42),
        ]
        return "_".join(f"{float(value):.6f}" for value in parts)

    def _hash_buffer_policy(self, buffer_policy: Optional[Dict[str, object]]) -> str:
        if not buffer_policy:
            return "none"
        enabled = bool(buffer_policy.get("enable", False))
        radius = float(buffer_policy.get("radius_m", buffer_policy.get("radius", 0.0)) or 0.0)
        remove_unlabeled_only = bool(buffer_policy.get("remove_unlabeled_only", True))
        return f"{int(enabled)}_{radius:.6f}_{int(remove_unlabeled_only)}"

    def _result_sort_key(self, result: Dict[str, object]):
        return primary_test_ei_sort_key(result)
