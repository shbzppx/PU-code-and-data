...
class ReportGenerator:
    ...
    def _model_context_rows(self, result) -> list[list[str]]:
        ...
        return [
            ["模型族", self._compact_value(context.get("model_family"))],
            ["输入类型", self._compact_value(context.get("input_kind"))],
            ["标签模式", self._compact_value(context.get("label_mode"))],
            ["训练模式", self._compact_value(context.get("training_mode"))],
            ["PU 模型", self._compact_value(context.get("is_pu_model"))],
            ["损失类型", self._compact_value(training_config.get("loss_type"))],
            ["先验模式", self._compact_value(training_config.get("prior_mode"))],
            ["先验概率", self._compact_value(training_config.get("prior"))],
            ["beta", self._compact_value(training_config.get("beta"))],
            ["gamma", self._compact_value(training_config.get("gamma"))],
            ["adaptive_window", self._compact_value(training_config.get("adaptive_window"))],
            ["adaptive_lambda", self._compact_value(training_config.get("adaptive_lambda"))],
            ["adaptive_gamma_min", self._compact_value(training_config.get("adaptive_gamma_min"))],
            ["adaptive_gamma_max", self._compact_value(training_config.get("adaptive_gamma_max"))],
            ["learning_rate", self._compact_value(training_config.get("learning_rate", training_config.get("lr")))],
            ["batch_size", self._compact_value(training_config.get("batch_size"))],
            ["optimizer", self._compact_value(training_config.get("optimizer"))],
            ["样本增强", self._compact_value(training_config.get("augmentation_enabled"))],
            ["扰动强度", self._compact_value(training_config.get("augmentation_noise_std"))],
        ]
    ...
    def _mineral_section_specs(self, result) -> list[tuple[str, str]]:
        if self._get_mineral_split_mode(result) in {"single", "spatial_region", "spatial_cluster"}:
            return [("train", "训练矿点"), ("test", "独立测试矿点")]
    ...
    def _overview_trial_mineral_lines(self, result) -> list[str]:
        summary = result.get("trial_scheme_summary") or {}
        stage1_trials = list(summary.get("stage1_trials") or [])
        lines = []
        if stage1_trials:
            stage1_total = int(summary.get("stage1_total", len(stage1_trials)) or len(stage1_trials))
            stage2_total = int(summary.get("stage2_total", 0) or 0)
            lines.append(f"Stage 1 扫描 {stage1_total} 个候选方案；Stage 2 复评 {stage2_total} 个方案")
            for item in stage1_trials[:10]:
                rank = item.get("rank")
                score = item.get("scheme_score")
                scheme_text = self._format_trial_scheme_brief(item.get("scheme") or {})
                prefix = f"{rank}. " if rank is not None else "- "
                try:
                    ...
                    suffix = f" | 方案评分 {self._compact_value(score)}" if score is not None else ""
                lines.append(f"{prefix}{scheme_text}{suffix}")
            if len(stage1_trials) > 10:
                lines.append("完整候选方案见下一页参数试验表格")
        else:
            lines.extend(self._trial_scheme_lines(result))

        split_summary = result.get("mineral_split_summary") or {}
        mineral_lines = []
        for key, title in self._mineral_section_specs(result):
            block = split_summary.get(key) or {}
            mineral_lines.append(f"{title}: {int(block.get('count', 0) or 0)} 个，完整清单见下一页")
        no_ore_block = split_summary.get("no_ore") or {}
        if no_ore_block:
            mineral_lines.append(f"无矿钻孔/坐标: {int(no_ore_block.get('count', 0) or 0)} 个，完整清单见下一页")
        if mineral_lines:
            lines.extend([""] + mineral_lines)
        return lines

    def _overview_dataset_rows(self, result) -> list[list[str]]:
        ...
        keys = [
            ...
            "negative_distance_multiplier",
            "train_mineral_count",
            "test_mineral_count",
            "train_sample_count",
            "test_sample_count",
        ]
        rows = [[key, dataset_params.get(key, "")] for key in keys if key in dataset_params]
        return rows or self._kv_rows(dataset_params)
    ...
    def _build_trial_summary_page(self, result):
        ...
        if split_mode == "single":
            subtitle = (
                f"Stage 1 扫描 {stage1_total} 个候选方案；Stage 2 复评 {stage2_total} 个方案 | "
                f"{train_title} {train_count} 个 | {second_title} {second_count} 个 | "
                "单文件自动分割（完整清单）"
            )
        ...
        self._render_table_section(
            ax_train,
            f"{train_title}完整清单（{train_count} 个）" if split_mode in {"single", "spatial_region", "spatial_cluster"} else f"{train_title}（{train_count} 个）",
            train_rows,
            col_labels=("预览",),
        ...
        self._render_table_section(
            ax_second,
            f"{second_title}完整清单（{second_count} 个）" if split_mode in {"single", "spatial_region", "spatial_cluster"} else f"{second_title}（{second_count} 个）",
            second_rows,
            col_labels=("预览",),
            font_size=min(mineral_table_font_size, 8.2),
            bbox=(0.0, 0.0, 1.0, 0.90),
    ...
    def _render_table_section(
        ...
        *,
        ...
        row_units = 1.25 + sum(max(1.0, float(line_count)) for line_count in row_line_counts)
        effective_font_size = float(font_size)
        if row_units > 24:
            effective_font_size = min(effective_font_size, 5.8)
        elif row_units > 18:
            effective_font_size = min(effective_font_size, 6.4)
        elif row_units > 14:
            effective_font_size = min(effective_font_size, 7.2)
        elif row_units > 10:
            effective_font_size = min(effective_font_size, 8.0)
        table.set_fontsize(effective_font_size)
    ...
    def _render_text_section(
        ...
        *,
        ...
        body = "\n".join(body_lines) if body_lines else empty_text
        line_count = max(1, len(body.splitlines()))
        effective_font_size = float(fontsize)
        if line_count > 18:
            effective_font_size = min(effective_font_size, 6.2)
        elif line_count > 14:
            effective_font_size = min(effective_font_size, 6.8)
        elif line_count > 10:
            effective_font_size = min(effective_font_size, 7.4)
        ax.text(
            0.02,
            0.96,
            body,
    ...
    def generate_pdf_report(self, output_path):
        ...
        best_result = max(self.results, key=primary_test_ei_score)
        best_ei_result = best_result

        with PdfPages(output_path) as pdf:
            overview_fig = self._build_overview_page(best_result=best_result, best_ei_result=best_ei_result)
    ...
    def _build_overview_page(self, *, best_result=None, best_ei_result=None):
        best_result = best_result or max(self.results, key=primary_test_ei_score)
        best_ei_result = best_ei_result or max(self.results, key=primary_test_ei_score)
        fig = plt.figure(figsize=(11.69, 8.27))
        gs = fig.add_gridspec(
            5,
            2,
        ...
        subtitle = (
            ...
            f"测试集EI: {primary_test_ei_score(best_result):.4f} | "
        ...
        summary_rows = [
            ...
            ["EI最优模型", best_ei_result.get("model_name", "-")],
            ["测试集EI", f"{primary_test_ei_score(best_result):.4f}"],
            ["综合评分（保留参考）", f"{best_result.get('composite_score', 0.0):.4f}"],
            ["CV EI均值", self._format_optional(best_result.get("cv_ei_mean"))],
            ["验证EI", self._format_optional(best_result.get("val_ei"))],
            ["测试EI", self._format_optional(best_result.get("test_ei"))],
            ["测试最优阈值", self._format_optional(best_result.get("best_test_threshold"))],
            ["测试最优阈值EI", self._format_optional(best_result.get("best_test_ei"))],
            ["验证准确率", self._format_optional(best_result.get("val_accuracy"))],
            ["样本增强", self._compact_value(best_result.get("dataset_params", {}).get("augmentation_enabled"))],
            ["扰动强度", self._compact_value(best_result.get("dataset_params", {}).get("augmentation_noise_std"))],
            ["内部验证矿点检出率", self._format_optional(best_result.get("val_mineral_detection_rate"))],
            ["独立测试矿点检出率", self._format_optional(best_result.get("test_mineral_detection_rate"))],
        ...
        self._render_text_section(
            ax_trials,
            "参数试验与矿点信息",
            self._overview_trial_mineral_lines(best_result),
            fontsize=7.6,
        )
        ...
        self._render_table_section(
            ax_dataset,
            "最佳数据方案",
            self._overview_dataset_rows(best_result),
            col_labels=("参数", "取值"),
            font_size=7.8,
            bbox=(0.0, 0.0, 1.0, 0.92),
            col_widths=(0.30, 0.66),
    ...
    def _build_ranking_chart_page(self):
        ...
        scores = [primary_test_ei_score(item) for item in self.results]
    ...
    def _build_ranking_table_page(self):
            ...
            [
                {
                    ...
                    "选择策略": item.get("selection_strategy", "composite"),
                    "测试集EI": f"{primary_test_ei_score(item):.4f}",
                    "综合评分（参考）": f"{item.get('composite_score', 0.0):.4f}",
                    "CV_EI": self._format_optional(item.get("cv_ei_mean")),
                    "验证EI": self._format_optional(item.get("val_ei")),
                    "测试EI": self._format_optional(item.get("test_ei")),
                    "测试最优阈值": self._format_optional(item.get("best_test_threshold")),
                    "测试最优阈值EI": self._format_optional(item.get("best_test_ei")),
                    "验证准确率": self._format_optional(item.get("val_accuracy")),
                    "样本增强": self._compact_value(item.get("dataset_params", {}).get("augmentation_enabled")),
                    "扰动强度": self._compact_value(item.get("dataset_params", {}).get("augmentation_noise_std")),
                    "内部验证矿点检出率": self._format_optional(item.get("val_mineral_detection_rate")),
                    "独立测试矿点检出率": self._format_optional(item.get("test_mineral_detection_rate")),
        ...
        col_widths = [0.10, 0.08, 0.08, 0.07, 0.07, 0.07, 0.07, 0.07, 0.08, 0.08, 0.08, 0.10, 0.10, 0.07, 0.07, 0.07, 0.08, 0.08, 0.08, 0.12]
        if len(col_widths) < len(columns):
            col_widths.extend([0.08] * (len(columns) - len(col_widths)))
    ...
    def _build_model_detail_page(self, result):
        ...
        subtitle = (
            f"测试集EI: {primary_test_ei_score(result):.4f} | "
            f"验证准确率: {self._format_optional(result.get('val_accuracy'))} | "
            f"独立测试准确率: {self._format_optional(result.get('test_accuracy'))}"
        )
        ax_title = fig.add_subplot(gs[0, :])
        self._render_page_header(ax_title, f"模型详情：{result.get('model_name', '-')}", subtitle)

        summary_rows = [
            ["测试集EI", f"{primary_test_ei_score(result):.4f}"],
            ["综合评分（参考）", f"{result.get('composite_score', 0.0):.4f}"],
            ["选择策略", str(result.get("selection_strategy", "composite"))],
            ["CV EI均值", self._format_optional(result.get("cv_ei_mean"))],
            ["CV EI标准差", self._format_optional(result.get("cv_ei_std"))],
            ["验证SR", self._format_optional(result.get("val_sr"))],
            ["验证PAF", self._format_optional(result.get("val_paf"))],
            ["验证EI", self._format_optional(result.get("val_ei"))],
            ["验证准确率", self._format_optional(result.get("val_accuracy"))],
            ["验证召回率", self._format_optional(result.get("val_recall", result.get("val_mineral_detection_rate")))],
            ["验证F1", self._format_optional(result.get("val_f1"))],
            ["样本增强", self._compact_value(result.get("dataset_params", {}).get("augmentation_enabled"))],
            ["扰动强度", self._compact_value(result.get("dataset_params", {}).get("augmentation_noise_std"))],
            ["内部验证矿点检出率", self._format_optional(result.get("val_mineral_detection_rate"))],
            ["测试SR", self._format_optional(result.get("test_sr"))],
            ["测试PAF", self._format_optional(result.get("test_paf"))],
            ["测试EI", self._format_optional(result.get("test_ei"))],
            ["测试召回率", self._format_optional(result.get("test_recall", result.get("test_mineral_detection_rate")))],
            ["测试最优阈值", self._format_optional(result.get("best_test_threshold"))],
            ["测试最优阈值SR", self._format_optional(result.get("best_test_sr"))],
            ["测试最优阈值PAF", self._format_optional(result.get("best_test_paf"))],
            ["测试最优阈值EI", self._format_optional(result.get("best_test_ei"))],
            ["SRC点数", str(len(result.get("src_curve") or []))],
            ["PAC点数", str(len(result.get("pac_curve") or []))],
            ["独立测试准确率", self._format_optional(result.get("test_accuracy"))],
            ["独立测试F1", self._format_optional(result.get("test_f1"))],
            ["独立测试矿点检出率", self._format_optional(result.get("test_mineral_detection_rate"))],
        ...
        summary_rows.extend(self._model_context_rows(result))
        ax_summary = fig.add_subplot(gs[1, :])
        self._render_table_section(
            ax_summary,
            "关键指标",
